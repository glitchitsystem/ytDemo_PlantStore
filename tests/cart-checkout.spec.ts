...
test.describe('Shopping Cart and Checkout', () => {
  ...
  test('Add a product to the cart and verify it appears', async ({ page }) => {
    ...
    const productName = await firstCard.locator('.product-name').first().textContent();
    ...
    await expect(page.locator('.cart-item h3')).toContainText(productName!.trim());
  });

  test('Update quantity to 2 and verify the total updates correctly', async ({ page }) => {
    await page.goto('/products');
    await page.waitForSelector('.product-card');
    ...
    await page.locator('.quantity-btn[aria-label="Increase quantity"]').first().click();
    await expect(page.locator('.quantity').first()).toHaveText('2');

    // Verify item total reflects quantity × price
    const itemTotalText = await page.locator('.cart-item-total').first().textContent();
    ...
    const summaryText = await page.locator('.cart-summary h3').filter({ hasText: 'Total:' }).textContent();
